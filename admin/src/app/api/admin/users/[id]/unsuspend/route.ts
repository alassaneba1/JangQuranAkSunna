...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    store.items[idx] = { ...store.items[idx], status: UserStatus.ACTIVE, updatedAt: now }
    return NextResponse.json({ data: store.items[idx], success: true, message: 'Réactivé', timestamp: now })
  } catch (e: any) {
    return NextResponse.json({ data: null, success: false, message: 'Erreur réactivation', timestamp: new Date().toISOString() }, { status: 500 })
  }
}
